...
def show_manual_entry_forms(master_order_id):
    ...
    with st.form("work_center_form"):
        ...
        wc_name = st.text_input("Work Center Name")
        ...
        machine_status = st.selectbox("Machine Status",
                                      ['OK', 'WARNING', 'CRITICAL', 'OFF'])
        available_date = st.date_input("Available From (Date)", value=date.today())
        available_time = st.time_input("Available From (Time)")

        submit_button = st.form_submit_button("Add Work Center")
                ...
                with db_session:
                    ...
                    if not work_center:
                        work_center = WorkCenter(
                            ...
                            work_center_description=wc_description
                        )
    ...
    with st.form("delivery_schedule_form"):
        scheduled_date = st.date_input("Scheduled Delivery Date", value=date.today())

        submit_button = st.form_submit_button("Add Delivery Schedule")
...
def save_to_database(data):
    with db_session:
        try:
            ...
            commit()  # Commit to get the master_order_id
                ...
                for op in data["Operations"]:
                    ...
                    if not work_center:
                        work_center = WorkCenter(
                            ...
                            work_center_description=op["Long Text"] if op["Long Text"] else op["Operation"]
                        )
...
def show_scheduling_options(master_order_id):
    st.subheader("Scheduling Options")

    with db_session:
        master_order = MasterOrder[master_order_id]

        # Create operations DataFrame for scheduling
        operations_data = []
        for op in master_order.operations:
            operations_data.append({
                'partno': master_order.part_number,
                'operation': op.operation_description,
                'machine': op.work_center.work_center_code,
                'sequence': op.operation_number,
                'time': op.per_piece_time
            })

        if operations_data:
            df = pd.DataFrame(operations_data)
            component_quantities = {master_order.part_number: master_order.required_quantity}

            if st.button("Generate Schedule"):
                schedule_df, end_time, total_time, daily_prod, status, partial = schedule_operations(
                    df, component_quantities
                )
                ...
                if not schedule_df.empty:
                    ...
                    st.write(f"Schedule End Time: {end_time}")
                    st.write(f"Total Time Required: {total_time:.2f} minutes")

                    if partial:
                        st.warning("Partial Completion Warnings:")
                        for warning in partial:
                            st.write(warning)
                else:
                    st.error("Could not generate schedule. Please check machine availability and raw materials.")
...
def main():
    ...
    if 'master_order_id' not in st.session_state:
        st.session_state.master_order_id = None

    uploaded_file = st.file_uploader("Choose a PDF file", type="pdf")

    if uploaded_file is not None:
        # Only process PDF and save to database if it hasn't been processed yet
        if not st.session_state.pdf_processed:
            ...
            try:
                ...
                st.session_state.master_order_id = save_to_database(data)
        ...
        # Show scheduling options and manual entry forms only if we have a master_order_id
        if st.session_state.master_order_id:
            show_scheduling_options(st.session_state.master_order_id)
            show_manual_entry_forms(st.session_state.master_order_id)
...
def show_manual_entry_forms(master_order_id):
    ...
    with st.form("delivery_schedule_form"):
        ...
        if submit_button:
            try:
                with db_session:
                    # Check if a delivery schedule already exists for this date
                    master_order = MasterOrder[master_order_id]
                    existing_schedule = DeliverySchedule.get(
                        order=master_order,
                        scheduled_delivery_date=scheduled_date
                    )

                    if existing_schedule:
                        st.warning("A delivery schedule already exists for this date.")
                    else:
                        DeliverySchedule(
                            order=master_order,
                            scheduled_delivery_date=scheduled_date
